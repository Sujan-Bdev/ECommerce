...
class UserCreateForm(forms.ModelForm):
    ...
    def save(self):
        ...
        m1=Customer.objects.create(user=new_user,location=self.cleaned_data.get('location'),contact=self.cleaned_data.get('contact'))
        m1.save()
        return new_user
...
class StaffCreateForm(forms.ModelForm):
    ...
    def clean(self):
        cleaned_data=super(AdminCreateForm,self).clean()
        username=cleaned_data.get('username')
        password=cleaned_data.get('password')
        password2=cleaned_data.get('password2')
        password_validation.validate_password(password=password,user=username)
